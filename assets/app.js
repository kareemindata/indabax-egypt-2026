...
function renderSpeakers(){
  ...
  $('speakerDirectory').innerHTML=list.map(s=>{const pr=s.status==='team_proposed'?'Team proposed':(s.priority||'Medium'); const pl=primaryLink(s); return `<div class="speaker-card${pl?' clickable':''}"${pl?` data-href="${esc(pl)}" role="link" tabindex="0" aria-label="Open profile for ${esc(s.name)}"`:''}><div class="spk-head">${speakerAvatar(s)}<div class="spk-id"><h3>${esc(s.name)}</h3><div class="title">${esc(s.title)}${s.org?', '+esc(s.org):''}</div></div><span class="rank">${s.rank||'•'}</span></div><div class="pill-row"><span class="pill ${priorityClass(pr)}">${esc(pr)}</span>${(s.focus||[]).slice(0,5).map(f=>`<span class="pill">${esc(f)}</span>`).join('')}</div><div class="fit">${esc(s.fit||'')}</div>${linksRow(s)}</div>`;}).join('')||'<div class="empty">No speakers match the selected filters.</div>';
}
